...
class WanTransformer3DModel(ModelMixin, ConfigMixin, FromOriginalModelMixin):
    ...
    def forward(
        ...
        emb_dtype = next(self.time_embedding.parameters()).dtype
        if t.dim() != 1:
            if t.size(1) < seq_len:
                pad_size = seq_len - t.size(1)
                last_elements = t[:, -1].unsqueeze(1)
            ...
            e = self.time_embedding(
                          sinusoidal_embedding_1d(self.freq_dim, t).to(emb_dtype)
                ) 
        ...
        if self.teacache is not None:
            if cond_flag:
                if t.dim() != 1:
                    ...
                    modulated_inp = e0
                #print('self.teacache.cnt', self.teacache.cnt) # 0, 1, 2
                #print('self.teacache.num_skip_start_steps', self.teacache.num_skip_start_steps) # 2

                #skip_flag = self.teacache.cnt < self.teacache.num_skip_start_steps
                #print('skip_flag', skip_flag) #true, true, false
                #print('self.teacache.accumulated_rel_l1_distance', self.teacache.accumulated_rel_l1_distance) # 0
                #print('self.teacache.rel_l1_thresh', self.teacache.rel_l1_thresh) # 0.1
                # print('cond_flag', cond_flag)  True
                if self.teacache.cnt == 0 or self.teacache.cnt == 1:
                    skip_flag = True
                else:
                    skip_flag = False
                ...
                if skip_flag:
                    ...
                    #if cond_flag: # jun for 3 steps turbo
                    #    rel_l1_distance = self.teacache.compute_rel_l1_distance(self.teacache.previous_modulated_input, modulated_inp)
                    #    self.teacache.accumulated_rel_l1_distance += self.teacache.rescale_func(rel_l1_distance)
                    #    #print(self.teacache.accumulated_rel_l1_distance , self.teacache.rel_l1_thresh)
    ...
    @classmethod
    def from_pretrained(
        cls, mode, pretrained_model_path, subfolder=None, transformer_additional_kwargs={},
        ...
        if low_cpu_mem_usage:
            try:
                ...
                if os.path.exists(model_file):
                    ...
                    from safetensors.torch import load_file, safe_open
                    if mode == 't2v':
                        model_files_safetensors = glob.glob(os.path.join(pretrained_model_path, "*_t2v.safetensors"))
                    elif mode == 'i2v':
                        model_files_safetensors = glob.glob(os.path.join(pretrained_model_path, "*_i2v.safetensors"))
                    state_dict = {}
